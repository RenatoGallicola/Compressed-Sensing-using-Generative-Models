...
The convolutional networks described above are our own design. The experiments of Bora et al. \cite{bora2017} on MNIST instead use a \textbf{fully connected} auto-encoder, with a recognition network of shape \( 784-500-500-k \) and a generator of shape \( k-500-500-784 \), a latent dimension of 20, and the same optimizer, batch size and learning rate we use. Since our generator differs from theirs, a difference between our results and the numbers they publish could be attributed either to the method or to the model.
\\

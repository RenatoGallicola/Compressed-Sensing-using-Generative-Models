...
Both networks are trained on the 54,000 images the VAE trains on, which is the training split with the same tenth held out, so that the two families see the same data and the test split takes no part in either. Training runs for 50 epochs with mini-batches of 64, one Adam optimizer for each network at a learning rate of 0.0002 with \( \beta_1 = 0.5 \), a binary cross-entropy loss, and two generator updates per discriminator update. These are the settings \cite{bora2017} give for their DCGAN in section 5.2, and they are also those of \cite{radford2016}, whose architecture that paper adopts and which identify \( \beta_1 = 0.9 \) as a cause of unstable training. Two versions were trained, one with a latent vector of dimension 20 and one of dimension 30.
\\
The two checkpoints used for the results in section 6 predate this configuration. They were trained on the training and test splits together, before the test split was held out, at a learning rate of 0.0001 with the default \( \beta_1 = 0.9 \), mini-batches of 32 and a single generator update per cycle, and no selection was applied among their epochs. The consequence for the two DCGAN columns is stated where those results are reported.

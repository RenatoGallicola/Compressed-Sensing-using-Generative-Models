...
\\
A generator is saved every five epochs from the twentieth, giving seven candidates per run, and the one used in section 6 is chosen among them by the rule in the repository's selection protocol: the lowest representation error over 32 images held out of training. The choice matters. The error does not fall monotonically with the epoch, it spans a factor of 1.28 at \( k = 20 \) and 1.42 at \( k = 30 \) across the saved candidates, and in both runs the final epoch was about 11\% worse than the one the rule selected. Keeping whatever the last epoch produced, which is the usual practice when a model has no validation loss, would have given a visibly weaker prior.

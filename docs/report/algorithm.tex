...
\end{equation}
Since both the VAE and the DCGAN impose an isotropic Gaussian prior on \( z \), the quantity \( \|z\|^2 \) is proportional to the negative log-likelihood of the latent vector under that prior, so the term keeps the search inside the region the generator was trained to cover. The authors report \( \lambda = 0.1 \) as the best value on MNIST, for their VAE, and we adopt it for every generative prior so that one protocol governs the whole table. That uniformity has a cost for the DCGAN, which the same authors run at 0.001 on their own dataset, and section 6.5 reports how much the comparison depends on it. Results obtained with \( \lambda = 0 \) are reported alongside, as the reference paper does.
\\

...
\text{loss}(z) = \| AG(z) - y \|^2.
\end{equation}
Bora et al. \cite{bora2017} also consider a regularized version of this objective, obtained by adding a term \( \lambda \|z\|^2 \) that keeps the solution inside the region where the prior \( P_Z \) places most of its mass. In our experiments we set \( \lambda = 0 \), so that the reconstruction depends on the generator alone.
\\
...
\\
In order to improve convergence we performed a thousand gradient steps for each of ten random restarts, and we kept the attempt that presented the smallest measurement error at the end of the approximation. These steps are not training epochs: the weights of \( G \) are frozen and the only variable that gets updated is the latent vector \( z \).

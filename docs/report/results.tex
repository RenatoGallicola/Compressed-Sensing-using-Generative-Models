...
\\\\
\textbf{The VAE beats the DCGAN at \( k = 20 \) everywhere, and at \( k = 30 \) only while measurements are scarce.} Against the DCGAN with latent dimension 20 every VAE is significantly more accurate at nine of the ten budgets. Against the one with latent dimension 30 the advantage holds from 10 to 75 measurements and then stops: from 300 upwards that generator has a lower mean error than any VAE, 0.0085 against 0.0069 with 750 measurements, although the difference does not reach significance with ten images. The two DCGANs differ from each other by more, at the larger budgets, than either differs from a VAE, which says more about how much a single adversarial training run varies than about the architecture.
\\\\
